...
(function (root, factory) {
  ...
  var EXTERNAL_LINKS = [
    ...
    { cat: '계산기 · 도구', name: '비공식 한월 스텟 계산기',
      url: 'https://79youngsoul.github.io/stat/',
      desc: '스텟 포인트 자동 분배 · 기대 데미지 계산 · 스크린샷 자동 인식 (이 사이트에 포함)' },
  ...
  var ASSISTANT_CRAFTS = [
    ...
    { name: '천옥완갑', mats: '취금완갑1 + 금강석3 + 흉폭한영기8 + 설화오금2', cost: 500000, p: 1.00,
      note: '탑오르기-5 퀘스트를 끝내야 제작 목록에 뜬다' }
  ];
  ...
  var SMITH_ACCESSORY = [
    ...
    { kind: '반지',   tier: '레벨 미확인', items: [{ name: '백호반지', stat: '옵션 미확인 · 검성 레이드 보상(7종 중 1개 랜덤)' }] },
    ...
    { kind: '귀걸이', tier: '170제', items: [{ name: '용문귀걸이', stat: '옵션 미확인 · 조복귀걸이 상위호환 · 운해궁 암토령 드랍' }] },
    { kind: '목걸이', tier: '100제', items: [
      { name: '적옥목걸이', stat: '생명력 +2' },
      { name: '청옥목걸이', stat: '힘 +2' },
      { name: '녹옥목걸이', stat: '행운 +2' }] },
    // 완갑 — 옵션 수치는 아직 모름. 제작 재료는 대장장이 조수 표(ASSISTANT_CRAFTS)에서 가져온다
    { kind: '완갑', tier: '조수 제작', items: [{ name: '정포완갑', stat: '옵션 미확인 · 황동완갑 제작 재료' }]
  ...
  // 대장간 제작대 장비제작 티어 확률
  // 제작창 왼쪽 상단부터 오른쪽으로 1 → 2 → 3 → 4 → 5티어 순서
  var GEAR_TIER_PROB = [
    { tier: '1티어', p: 5 }, { tier: '2티어', p: 10 }, { tier: '3티어', p: 25 },
    { tier: '4티어', p: 30 }, { tier: '5티어', p: 30 }
  ];
  var GEAR_TIER_NOTE = '50레벨 장비 제작부터 티어 존재. 제작창 왼쪽 상단 → 오른쪽 순서로 1·2·3·4·5티어 '
    + '(예: 2번째 줄 2번째 장비 = 5티어). 아래 장비 목록도 같은 순서로 정렬되어 있음.';

  // 단순 "아이템 → 확률(%)" 표들
  var DROP_TABLES = [
    {
      title: '토벌의뢰 아이템 획득 확률',
      note: '운해궁 토벌의뢰는 다른 던전보다 토벌의뢰 추가 경험치가 높다(2026-08-29 추가). 아이템 확률은 별도 고지가 없어 아래 표와 같은 것으로 본다.',
      rows: [
  ...
  var BOSS_RAIDS = [
    {
      boss: '장로쥐',
      note: '아래 7종 중 1개 랜덤 · 개별 확률 미고지 · 괴암곡제단에서 소환',
      rewards: ['금제반지', '마군반지', '이류주문서상자', '반지주문서상자', '금전', '은괴', '은환단']
    },
    {
      boss: '검성',
    ...
    {
      boss: '오공',
      note: '아래 8종 중 1개 랜덤 · 개별 확률 미고지',
      rewards: ['명월단', '화심반지', '황금반지', '은괴', '일반부적뽑기', '이류주문서상자', '반지주문서상자', '금전']
    }
  ];
  ...
  var HUNTING_GROUNDS = [
    ...
    { name: '운해궁', lv: '185~195', x: -1450, y: 156, z: 5572, monsters: '암토령(Lv.185), 금갑수(Lv.190), 염각귀(Lv.195)',
      memo: '2026-08-29 몬스터 데미지 감소 · 토벌의뢰 추가(다른 던전보다 토벌의뢰 추가 경험치가 높음) / 겉날개 수정은 10·11채 선적용',
      drops: [
        { n: '암령토', from: '암토령', note: '추후 상위 완갑 업그레이드 재료' },
        { n: '용문귀걸이', from: '암토령', note: '170제 귀걸이 · 조복귀걸이 상위호환' }
      ] },
